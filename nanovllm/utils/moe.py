...
def moe_align_block_size_fixed(topk_ids, num_experts, block_size, expert_map=None):
    """Graph-safe fixed-capacity MoE token alignment.

    CUDA graph capture cannot depend on a GPU-computed ``num_blocks.item()`` or
    allocate route-dependent output sizes. This variant reserves the same number
    of blocks for every expert and pads unused slots with the sentinel token id.
    """
    device = topk_ids.device
    M, top_k = topk_ids.shape
    num_total_tasks = M * top_k
    max_blocks_per_expert = (num_total_tasks + block_size - 1) // block_size
    ...
    sort_idx = torch.argsort(flat_topk_ids)
    sorted_expert_ids = flat_topk_ids[sort_idx]
    sorted_tokens_raw = flat_token_ids[sort_idx]
    sorted_weight_idx_raw = flat_weight_idx[sort_idx]

    # ``torch.bincount`` is not capture-safe (it host-syncs to size the output),
    # so use a static-shape scatter_add for the per-expert token count.
    tokens_per_expert = torch.zeros(num_experts, dtype=torch.int64, device=device)
    tokens_per_expert.scatter_add_(
        0, flat_topk_ids.long(), torch.ones_like(flat_topk_ids, dtype=torch.int64)
    )
    expert_offsets = torch.cumsum(tokens_per_expert, dim=0) - tokens_per_expert
    sorted_pos = torch.arange(num_total_tasks, device=device)
    rank_within_expert = sorted_pos - expert_offsets[sorted_expert_ids]
    padded_target_pos = (
        sorted_expert_ids * max_blocks_per_expert * block_size + rank_within_expert
    )
    ...
    expert_ids = torch.arange(num_experts, device=device).repeat_interleave(max_blocks_per_expert).to(torch.int32)
    if expert_map is not None:
        expert_ids = expert_map[expert_ids.long()].to(torch.int32)
